...
ParseScript(std::string s)
{
    ...
    BOOST_FOREACH(std::string w, words)
    {
        ...
        else if (boost::algorithm::starts_with(w, "0x") && IsHex(std::string(w.begin()+2, w.end())))
        {
...
BOOST_AUTO_TEST_CASE(script_valid)
{
    ...
    // scripts.
    json_spirit::Array tests = read_json("script_valid.json");
...
BOOST_AUTO_TEST_CASE(script_invalid)
{
    ...
    json_spirit::Array tests = read_json("script_invalid.json");
