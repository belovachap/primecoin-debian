...
int NetworkPeerManager::GetTriedBucket(const NetworkPeer &network_peer, const std::vector<unsigned char> &nKey) const
{
...
int NetworkPeerManager::GetNewBucket(const NetworkPeer &network_peer, const std::vector<unsigned char> &nKey, const CNetAddr& src) const
{
    CDataStream ss1(SER_GETHASH, 0);
    std::vector<unsigned char> vchGroupKey = network_peer.GetGroup();
...
bool NetworkPeerManager::IsTerrible(const NetworkPeer &network_peer, int64 nNow) const
{
